...
def _derive_csv_path(base, cosmo):
    ...
    if ext.lower() == '.csv':
        # Cas où 'base' est un fichier (ex: "path/to/stats.csv")
        # Nouveau chemin: "path/to/wst_mean/stats_{cosmo}.csv"
...
def compute_wst_S012(
    ...
    Automatically retries on CPU if GPU execution fails (e.g., CUDA arch mismatch).
    """
    ...
    # 5) Compute (with safe GPU→CPU fallback)
    try:
        ...
        dev_effective = st_kwargs.get("device", "auto")
    except RuntimeError as e:
        msg = str(e)
        needs_cpu = (
            "no kernel image is available" in msg
            or "CUDA error" in msg
            or "device-side assertions" in msg
            or "invalid device" in msg
        )
        if (dev in {"auto", "gpu"}) and needs_cpu:
            if not quiet:
                print("[WST] CUDA failed; retrying on CPU...")
            # Ensure CUDA is ignored when we retry
            os.environ.setdefault("CUDA_LAUNCH_BLOCKING", "1")
            os.environ["CUDA_VISIBLE_DEVICES"] = ""
            st_kwargs_cpu = dict(st_kwargs)
            st_kwargs_cpu["device"] = "cpu"
            st_calc = scattering.Scattering2d(M, N, J, L, **st_kwargs_cpu)
            s_mean = st_calc.scattering_coef(x)
            s_cov  = st_calc.scattering_cov (x)
            dev_effective = "cpu"
        else:
            raise
    ...
    # 7) Convert -> 2D numpy
    S1 = _to_numpy(S1_t).reshape(x.shape[0], -1)
    S2 = None if S2_t is None else _to_numpy(S2_t).reshape(x.shape[0], -1)

...
def _setup_julia_env(envdir: str,
                     xgpaint_url: str = "https://github.com/licongxu/XGPaint.jl",
                     xgpaint_rev: Optional[str] = None,
                     quiet: bool = False) -> None:
    """
    Prepare the Julia project (without using juliacall yet).
    Ensures PythonCall, XGPaint and required deps are present & precompiled.
    """
    os.makedirs(envdir, exist_ok=True)
    os.environ["JULIA_PROJECT"] = envdir  # will be used by juliacall later

    rev_line = f', rev="{xgpaint_rev}"' if xgpaint_rev else ""
    code = f"""
    import Pkg
    Pkg.activate(raw"{envdir}"; shared=false)
    try; Pkg.Registry.add("General"); catch; Pkg.Registry.update(); end
    Pkg.add(["PythonCall","CSV","DataFrames","Healpix","Pixell","HDF5","HTTP","OpenSSL","ImageFiltering"])
    try; Pkg.rm("XGPaint"; force=true); catch; end
    Pkg.develop(Pkg.PackageSpec(url="{xgpaint_url}"{rev_line}))
    Pkg.instantiate(); Pkg.precompile()
    """
    subprocess.run(["julia", "--startup-file=no", "-e", code],
                   check=True, text=True, capture_output=quiet)


def _import_juliacall():
    """
    Import juliacall after JULIA_PROJECT is prepared.
    """
    from juliacall import Main as jl
    return jl


def _ensure_julia_symbols(jl) -> None:
    """
    Define Julia-side helpers and make sure the required modules are loaded.
    """
    jl.seval(r"""
    ...
    function paint_one_catalogue(cat_path::AbstractString,
        ...
        # Read catalogue (z, M, lon, lat), build model, etc.
        ...
        # Safe conversion to Float32: Array(...) then rebuild Enmap
        buf32      = Float32.(Array(sky_map64))   # Matrix{Float32}
        sky_map32  = Enmap(buf32, wcs)            # Enmap{Float32, …}
...
def paint_patches(
    ...
    *,
    ...
    # 1) Prepare Julia project (no juliacall yet)
    if envdir is None:
    ...
    # 2) Only now import juliacall (it will pick up JULIA_PROJECT)
    jl = _import_juliacall()

    # Optional: quick env info for logs
    jl.seval(r"""
    ...
    println()
    println("── Pkg.status ──────────────────────────────────────")
    Pkg.status(["XGPaint","Healpix","Pixell","HDF5","HTTP","OpenSSL","PythonCall"])
    println("────────────────────────────────────────────────────")
    """)

    # 3) Ensure Julia-side functions are defined
    _ensure_julia_symbols(jl)

    # 4) Validate & expand catalogue inputs
    if catalogs is None:
    ...
    cat_list = _csvs_from_paths(cand, recursive=bool(recursive), require_params_in_title=True)
    if not cat_list:
    ...
    # 5) Prepare manifest bookkeeping
    outs: List[str] = []
    records: List[dict] = []
    ...
    with tempfile.TemporaryDirectory(prefix="tsz_params_") as tmpdir:
        for i, cfile in enumerate(cat_list):
            ...
            if par_list[i] is None:
                try:
                    from title_reader import extract_params  # optional helper
    ...
    # 6) Merge/append manifest and print total runtime (in seconds)
    try:
